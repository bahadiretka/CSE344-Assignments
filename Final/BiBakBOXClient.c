...
/*
Data flow is provided from the server to the client.
If the file does not exist in the client, the file is taken from the server or deleted from the server.
*/
void server_to_client(int fd, char* directory,int flag){
...
/*
Data flow is provided from client to server.
The client directory is browsed and the names and modification dates of the files are burned into the socket.
If the file does not exist on the server or if the modified date is older, the file is copied to the server.
*/
int client_to_server(int fd, char* directory){
...
//Path is parsed to find the file name.
char* pathname(char * path){
